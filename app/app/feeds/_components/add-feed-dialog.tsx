...
import { Check, Loader2, Plus, Rss, X as XIcon } from "lucide-react";
...
export function AddFeedDialog({
	...
	const [tab, setTab] = useState<CuratedCategory>(CURATED_CATEGORIES[0]);
	...
	return (
		...
		>
			...
			<Dialog.Portal>
				...
				<Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[min(820px,calc(100vw-2rem))] max-h-[min(720px,calc(100vh-2rem))] -translate-x-1/2 -translate-y-1/2 flex flex-col rounded-3xl border border-border bg-background-elev shadow-xl outline-none data-starting-style:opacity-0 data-starting-style:scale-95 transition-[opacity,transform] duration-200">
					<header className="flex items-start gap-3 p-6 pb-4">
						...
						<div className="flex-1">
							...
							<Dialog.Description className="mt-0.5 text-[12.5px] text-ink/60 leading-normal">
					...
					<div className="px-6 pb-5">
						<form onSubmit={handleCustomSubmit} className="space-y-2">
							<label className="block text-[11px] font-semibold uppercase tracking-[0.18em] text-ink/55">
								Paste a feed URL
							</label>
							...
							{customError ? (
								<p className="text-[12.5px] text-primary-deep">{customError}</p>
							) : customSuccess ? (
								...
								<p className="text-[12px] text-ink/55 leading-normal">
									RSS or Atom. Import can take 5–15 seconds for slow publishers.
								</p>
							)}
						</form>
					</div>
					...
					<div className="px-6 pb-6 min-h-0 flex-1 flex">
						<section className="pt-2 flex flex-col min-h-0 flex-1 rounded-2xl border border-border bg-background overflow-hidden">
							<FilterTabs
								className="px-3"
								activeKey={tab}
								onSelect={(k) => setTab(k as CuratedCategory)}
								items={CURATED_CATEGORIES.map((c) => ({
									key: c,
									label: c,
									count: catalog[c].length,
								}))}
							/>
							<div className="min-h-0 flex-1 overflow-y-auto p-3">
								<ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
									{catalog[tab].map((feed) => (
										<FeedRow
											key={feed.url}
											feed={feed}
	...
	return (
		<li className="flex items-start justify-between gap-3 rounded-2xl border border-border bg-background p-4">
			<div className="flex items-start gap-3 min-w-0 flex-1">
				<FeedAvatar title={feed.title} siteUrl={feed.siteUrl} size={32} />
