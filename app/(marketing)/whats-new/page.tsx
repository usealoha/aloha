...
const RELEASES: Release[] = [
  {
    ...
    lead:
      "Six months of creator testing later, the automation canvas is ready for everyone. Human-approve default, cross-channel triggers, the whole thing.",
    changes: [
      { tag: "new", t: "Logic Matrix available on Pro and Agency — free matrices included on the free plan up to three nodes." },
      { tag: "new", t: "Six starter matrix templates shipped with the app." },
      { tag: "improved", t: "Dry-run preview now shows a 7-day simulation instead of just the next trigger." },
      { tag: "fixed", t: "Fixed a race condition where two matrices on the same trigger could double-run." },
    ],
    featured: true,
  ...
  },
  {
    ...
    lead:
      "The voice model learns from your best posts, not your whole archive. Users in the beta cohort saw a jump from 78% to 94% voice-match on average.",
    changes: [
      { tag: "improved", t: "New training method — pick the 12 posts that sound most like you, not the last 100." },
      { tag: "improved", t: "Voice settings UI: tone sliders are labelled in plain English ('short sentences' / 'long sentences' instead of 'tau = 0.3')." },
      { tag: "new", t: "'Try a rewrite' button in voice settings — paste a draft, see how the current model would rewrite it for each channel." },
    ],
  },
  {
    version: "1.12",
    ...
    lead:
      "Comments, DMs, and mentions are now sorted into three buckets. The daily digest email summarises everything you didn't reply to and why.",
    changes: [
      { tag: "new", t: "Inbox triage view (Questions / Praise / Needs-review)." },
      { tag: "new", t: "Daily digest email — opt in, opt out, one click either way." },
      { tag: "improved", t: "'Low-touch' classifier can now be taught per-account." },
    ],
  },
  {
    version: "1.11",
    ...
    lead:
      "Two new channels moved out of beta. Pinterest supports pins and boards; Threads uses the native cross-post API from Instagram (not a screenshot repost).",
    changes: [
      { tag: "new", t: "Pinterest scheduling — pins and boards, with image + description native fields." },
      { tag: "new", t: "Threads auto-mirror from Instagram, using the platform's native cross-post." },
      { tag: "improved", t: "Channel connect flow unified across all 8 networks." },
      { tag: "fixed", t: "TikTok drafts would occasionally lose their cover frame — fixed." },
    ],
  },
  {
    version: "1.10",
    ...
    lead:
      "Calendar now calls out the things you'd want a careful editor to call out — two-post Tuesdays, a channel that's been silent for three days, a launch beat that slipped.",
    changes: [
      { tag: "new", t: "Conflict chips: two-post-per-hour, channel-silence, launch-beat-missing." },
      { tag: "new", t: "Cadence settings — per-channel weekly frequency, quiet hours, preferred slots." },
      { tag: "improved", t: "Drag-to-reschedule snaps to best-time by default; override with Shift." },
    ],
  },
  {
    version: "1.9",
    ...
    lead:
      "Every plan, including Free, now gets 24 months of analytics history and CSV export. No paywall on your own data.",
    changes: [
      { tag: "new", t: "CSV, JSON, and Markdown digest exports on every view." },
      { tag: "new", t: "24-month history on every plan, including Free." },
      { tag: "improved", t: "Platform-API gaps surface as visible markers instead of silent zeros." },
    ],
  },
];
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            <div>
              ...
              <h1 className="font-display font-normal text-ink leading-[0.98] tracking-[-0.03em] text-[56px] sm:text-[68px] lg:text-[88px]">
                What's new.
                <br />
                <span className="italic text-primary font-light">In order.</span>
              </h1>
              <p className="mt-8 max-w-xl text-[16px] lg:text-[17px] leading-[1.55] text-ink/70">
                Every shipped feature, improvement, and fix — dated
                honestly, written in English. Release notes without the
                marketing voice.
              </p>
            </div>
          ...
          <article className="rounded-3xl bg-primary-soft border border-primary/15 overflow-hidden">
            <div className="p-8 lg:p-12 grid grid-cols-12 gap-8 lg:gap-12">
              <div className="col-span-12 lg:col-span-5">
                ...
                <time dateTime={featured.date} className="mt-3 block text-[12.5px] font-mono text-ink/60">
                  {featured.dateLabel}
                </time>
                  ...
                  {featured.changes.map((c, i) => {
                    ...
                    return (
                      <li key={i} className="flex items-start gap-3">
                        <span className={`inline-flex items-center gap-1 text-[9px] font-semibold uppercase tracking-[0.18em] ${bg} px-2 py-0.5 rounded-full shrink-0 mt-[3px]`}>
                          <Icon className="w-2.5 h-2.5" />
                          {c.tag}
                        </span>
                        <p className="text-[14px] text-ink/85 leading-[1.5]">{c.t}</p>
                      </li>
                    );
                  })}
                </ul>
                      ...
                      {r.changes.map((c, i) => {
                        ...
                        return (
                          <li key={i} className="flex items-start gap-3">
                            ...
                            <p className="text-[13.5px] text-ink/85 leading-[1.55]">{c.t}</p>
                          </li>
                        );
                      })}
                    </ul>
          ...
          <div className="grid grid-cols-12 gap-10 items-center">
            <div className="col-span-12 lg:col-span-7">
              <h2 className="font-display text-[32px] lg:text-[44px] leading-[1.05] tracking-[-0.015em]">
                ...
                <br />
                <span className="italic text-primary">You don't have to read every one.</span>
              </h2>
              <p className="mt-5 max-w-lg text-[15px] text-ink/70 leading-[1.6]">
                Prefer a monthly wrap in your inbox instead? We send a plain-
                English summary on the first of every month. No marketing
                fluff, no "exciting news."
              </p>
            </div>
